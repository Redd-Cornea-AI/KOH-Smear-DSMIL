# this is not usable for tree features
...
def test(args, bags_list, milnet):
    ...
    print('Number of bags: ' + str(num_bags))
    ...
    for i in range(0, num_bags):
        ...
        with torch.no_grad():
            try:
                ...
                for c in range(args.num_classes):          
                    if bag_prediction[c] >= args.thres[c]:
                        ...
                        if benign: # first class detected
                            print(bags_list[i] + ' is detected as: ' + args.class_name)
                ...
                # normalize colored tiles to have variance 1
                colored_tiles = (colored_tiles - colored_tiles.mean()) / colored_tiles.std()
                # Normalize colored tiles to the range [0, 255]
                colored_tiles = (colored_tiles / colored_tiles.max()) * 255
                colored_tiles = exposure.rescale_intensity(colored_tiles, out_range=(0, 255))
                color_map = np.zeros((np.amax(pos_arr, 0)[0]+1, np.amax(pos_arr, 0)[1]+1, 3), dtype=np.uint8)
                for k, pos in enumerate(pos_arr):
                    color_map[pos[0], pos[1]] = colored_tiles[k]
                slide_name = bags_list[i].split(os.sep)[-1]
                color_map = transform.resize(color_map, (color_map.shape[0]*32, color_map.shape[1]*32), order=0, preserve_range=True, anti_aliasing=False)
                io.imsave(os.path.join(args.map_path, slide_name+'.png'), img_as_ubyte(color_map))
                print('Color map for ' + slide_name + ' is saved.')
                ...
                print('Error: ' + str(e))
                print('Error in processing ' + bags_list[i])
                continue
...
if __name__ == '__main__':
    ...
    parser.add_argument('--thres', nargs='+', type=float, default=[0.5693772435188293, ])
    parser.add_argument('--class_name', nargs='+', type=str, default='Fungus Positive')
    parser.add_argument('--embedder_weights', type=str, default='test/test_lowmag/embedder_low.pth')
    parser.add_argument('--aggregator_weights', type=str, default='test/test_lowmag/mil_weights_fold_2.pth')
    parser.add_argument('--bag_path', type=str, default='test/test_bags/Fungal_Positive')
    # parser.add_argument('--bag_path', type=str, default='WSI/KOH_Dataset_test_lambda/pyramid/Fungal_Negative')
    ...
    b_classifier = mil.BClassifier(input_size=args.feats_size, output_class=args.num_classes, nonlinear=args.non_linearity).cuda()
