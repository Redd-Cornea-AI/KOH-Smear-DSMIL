# In v2, we select only slides with 0 and 1
# In v3, we exclude classes 2&3 (v2) then we merge the same patients together
# In v4, we drop patients duplicates randomly rather than merging them
# In v5, we create a way to include test set from an external source in '5-fold-cv-standalone-test' mode
...
def combine_csv_files(src_dir, dst_dir, args):
    ...
    # # recreate main csv with classes of the dataset
    # # enumerate folders in root and create a df of the csv file names present in each folder
    # # folder names in order will be classified as 0, 1, 2, 3, ...
    # df = pd.DataFrame()
    # for root, dirs, files in os.walk(dst_dir):
    #     for i, folder in enumerate(dirs):
    #         folder_path = os.path.join(root, folder)
    #         for file in os.listdir(folder_path):
    #             # list all csv files names in the folder
    #             if file.endswith('.csv'):
    #                 new_row = pd.DataFrame({'0': [os.path.join(folder_path, file)], 'label': [i]})
    #                 df = pd.concat([df, new_row], ignore_index=True)
                        
    #         # save the df to a csv file
    #         df.to_csv(os.path.join(dst_dir, args.dataset+'_combined.csv'), index=False)
...
def main():
    ...
    #             # test_loss_bag, avg_score, aucs, thresholds_optimal, predictions, labels = test(args, test_path, milnet, criterion, return_predictions=True)
    #             test_loss_bag, avg_score, aucs, thresholds_optimal, predictions, labels, files = test(args, test_path, milnet, criterion, return_predictions=True, return_files=True)
    ...
    #                 best_files = copy.deepcopy(files)
                    
    #             if counter > args.stop_epochs: break
    #         fold_results.append((best_ac, best_auc))
            
    #         # Store predictions and labels
    #         all_predictions.extend(best_predictions.tolist())
    #         all_labels.extend(best_labels.tolist())
    ...
    #     # Save all predictions and labels to CSV
    #     results_df = pd.DataFrame({
    #         'Predictions': all_predictions,
    #         'Labels': all_labels,
    #         'Files': all_files
    #     })
    #     results_df.to_csv('test/train_predictions.csv', index=False)
    ...
    if args.eval_scheme == '5-fold-cv-standalone-test':
        ...
        # results_df.to_csv('test/test_predictions.csv', index=False)
        # # save with current date
        # results_df.to_csv(f'test/test_predictions_{datetime.date.today().strftime("%Y%m%d")}.csv', index=False)
