...
def remap_labels_v3(label_value):
    """
    V3 Label remapping function:
    - Class 1 (fungal) remains as 1
    - Classes 0, 2, 3 (non-fungal, uncertain, indeterminate) become 0
    """
    return 1 if label_value == 1 else 0

def main():
    parser = argparse.ArgumentParser(description='Train DSMIL on 10x and 20x patch features learned by SimCLR - V3: Class 1 vs All Others')
    ...
    print(f"V3 Training: Class 1 (fungal) vs All Others (0,2,3) combined")
    ...
    #######################
    # V3 MODIFICATION: Keep ALL classes (0,1,2,3) and remap them
    print("=== V3 TRAINING SET PROCESSING ===")
    ...
    # V3 KEY CHANGE: Instead of filtering, remap all classes
    # Keep ALL classes (0,1,2,3) and remap them:
    # - Class 1 (fungal) stays as 1
    # - Classes 0,2,3 (non-fungal, uncertain, indeterminate) become 0
    print("Original label distribution:")
    print(df_merged['label'].value_counts(dropna=False).sort_index())
    
    df_merged['label'] = df_merged['label'].apply(remap_labels_v3)
    
    print("After V3 remapping (1 vs all others):")
    print(df_merged['label'].value_counts(dropna=False).sort_index())
    
    # save new df as csv
    df_merged.to_csv(os.path.join('datasets', args.dataset, args.dataset+'_v3_edited.csv'), index=False)
    print('CSV length after V3 editing: ', len(df_merged))
    # define new bags_csv
    bags_csv = os.path.join('datasets', args.dataset, args.dataset+'_v3_edited.csv')
    #######################
    ...
    bags_path.to_csv('merged_patients_v3.csv', index=False)
    
    # Create the new directory name by appending '_combined'
    new_dataset_dir = 'datasets/' + args.dataset + '_v3_combined'
    ...
    print(f"V3 Dataset cloned and combined at: {new_dataset_dir}")
    
    bags_csv = os.path.join(new_dataset_dir,args.dataset+'_v3_combined.csv')
    ...
    #######################
    # V3 MODIFICATION: Keep ALL classes (0,1,2,3) and remap them for test set
    print("=== V3 TEST SET PROCESSING ===")
    df_annotated = pd.read_csv('Consolidated_annotations_KOHsmear.csv')
    # keep only "File name" and "Merged_numerical" columns
    df_annotated = df_annotated[['File name', 'Merged_numerical']]
    # remove "_cropped.jpeg" from "File name" end
    ...
    # V3 KEY CHANGE: Instead of filtering, remap all classes for test set
    print("Original test label distribution:")
    print(df_merged['label'].value_counts(dropna=False).sort_index())
    
    df_merged['label'] = df_merged['label'].apply(remap_labels_v3)
    
    print("After V3 test remapping (1 vs all others):")
    print(df_merged['label'].value_counts(dropna=False).sort_index())
    
    # save new df as csv
    df_merged.to_csv(os.path.join('datasets', args.dataset_test, args.dataset_test+'_v3_edited.csv'), index=False)
    print('Test CSV length after V3 editing: ', len(df_merged))
    # define new bags_csv
    bags_csv_test = os.path.join('datasets', args.dataset_test, args.dataset_test+'_v3_edited.csv')
    #######################
    ...
    print('Merged same test entries: ')
    print(bags_path_test.head())
    # print value counts without excluding nans
    print('Test value counts: ')
    print(bags_path_test['label'].value_counts(dropna=False))
    # save the new csv for logging
    bags_path_test.to_csv('merged_patients_v3_test.csv', index=False)
    
    # Create the new directory name by appending '_combined'
    new_dataset_dir = 'datasets/' + args.dataset_test + '_v3_combined'
    # Clone the directory structure of the original dataset
    ...
    print(f"V3 Test dataset cloned and combined at: {new_dataset_dir}")
    
    bags_csv_test = os.path.join(new_dataset_dir,args.dataset_test+'_v3_combined.csv')
    ...
    if args.eval_scheme == '5-fold-cv-standalone-test':
        ...
        for fold, (train_index, test_index) in enumerate(kf.split(bags_path)):
            print(f"Starting V3 CV fold {fold}.")
        ...
        print("Starting V3 testing phase.")
        fold_predictions = []
        ...
        print("V3 Mean AUC:", mean_auc)
        print("V3 Fold results on CV (accuracy + auc):", fold_results)
        
        print("V3 Fold predictions shape:", np.array(fold_predictions).shape)
        print("V3 Mode result:", mode_result)
        print("V3 Combined predictions before squeeze:", mode_result.mode[0])
        print("V3 Combined predictions:", combined_predictions)
        ...
        fold_predictions_df.to_csv(f'test/test_predictions_v3_per_fold_{datetime.datetime.now().strftime("%Y%m%d_%H%M")}.csv', index=False)
        ...
        results_df.to_csv(f'test/test_predictions_v3_mode_{datetime.datetime.now().strftime("%Y%m%d_%H%M")}.csv', index=False)

        if args.num_classes > 1:
            # Compute Hamming Loss
            hammingloss = hamming_loss(test_labels, combined_predictions)
            print("V3 Hamming Loss:", hammingloss)
            # Compute Subset Accuracy
            subset_accuracy = accuracy_score(test_labels, combined_predictions)
            print("V3 Subset Accuracy (Exact Match Ratio):", subset_accuracy)
        else:
            accuracy = accuracy_score(test_labels, combined_predictions)
            print("V3 Accuracy:", accuracy)
            balanced_accuracy = balanced_accuracy_score(test_labels, combined_predictions)
            print("V3 Balanced Accuracy:", balanced_accuracy)

        os.makedirs('test', exist_ok=True)
        with open("test/test_list_v3.json", "w") as file:
            json.dump(reserved_testing_bags, file)

        for i, item in enumerate(fold_models):
            best_model = item[0]
            optimal_thresh = item[1]
            torch.save(best_model.state_dict(), f"test/mil_weights_v3_fold_{i}.pth")
            with open(f"test/mil_threshold_v3_fold_{i}.json", "w") as file:
