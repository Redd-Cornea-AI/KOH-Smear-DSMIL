# In this version (v3), we train class 1 (fungal) vs all other classes (0, 2, 3) combined as negative class
# This provides a more inclusive training approach that uses all available data
# Key changes from v2:
# - Keep all classes (0, 1, 2, 3) instead of filtering out 2 and 3
# - Remap classes 0, 2, 3 to 0 (negative/non-fungal)
# - Keep class 1 as 1 (positive/fungal)
...
def save_model(args, fold, run, save_path, model, thresholds_optimal):
    ...
    save_name = os.path.join(save_path, f'fold_{fold}_run_{run}.pth')
    torch.save(model.state_dict(), save_name)
    
    # Save thresholds as well
    thresholds_path = os.path.join(save_path, f'thresholds_fold_{fold}_run_{run}.json')
    with open(thresholds_path, 'w') as f:
        json.dump([float(t) for t in thresholds_optimal], f)
...
def remap_labels_v4(label_value):
    """
    V4 Label remapping function:
    - Classes 0, 1 (good quality) become 1
    - Classes 2, 3 (poor quality) become 0
    """
    return 1 if label_value in [0, 1] else 0

def main():
    parser = argparse.ArgumentParser(description='Train DSMIL on 10x and 20x patch features learned by SimCLR - V4: Quality Classification')
    ...
    print(f"V4 Training: Quality Classification - Good (0,1) vs Poor (2,3)")
    print(f"Evaluation scheme: {args.eval_scheme}")
    ...
    #######################
    # V4 MODIFICATION: Keep ALL classes (0,1,2,3) and remap them for quality
    print("=== V4 TRAINING SET PROCESSING (QUALITY CLASSIFICATION) ===")
    ...
    # V4 KEY CHANGE: Instead of filtering, remap all classes for quality
    # Keep ALL classes (0,1,2,3) and remap them:
    # - Classes 0,1 (good quality) become 1
    # - Classes 2,3 (poor quality) become 0
    print("Original label distribution:")
    print(df_merged['label'].value_counts(dropna=False).sort_index())
    
    df_merged['label'] = df_merged['label'].apply(remap_labels_v4)
    
    print("After V4 remapping (good vs poor quality):")
    print(df_merged['label'].value_counts(dropna=False).sort_index())
    
    # save new df as csv
    df_merged.to_csv(os.path.join('datasets', args.dataset, args.dataset+'_v4_edited.csv'), index=False)
    print('CSV length after V4 editing: ', len(df_merged))
    # define new bags_csv
    bags_csv = os.path.join('datasets', args.dataset, args.dataset+'_v4_edited.csv')
    #######################
    ...
    bags_path.to_csv('merged_patients_v4.csv', index=False)
    
    # Create the new directory name by appending '_combined'
    new_dataset_dir = 'datasets/' + args.dataset + '_v4_combined'
    ...
    print(f"V4 Quality Dataset cloned and combined at: {new_dataset_dir}")
    
    bags_csv = os.path.join(new_dataset_dir,args.dataset+'_v4_combined.csv')
    ...
    #######################
    # V4 MODIFICATION: Keep ALL classes (0,1,2,3) and remap them for test set
    print("=== V4 TEST SET PROCESSING (QUALITY CLASSIFICATION) ===")
    df_annotated = pd.read_csv('Consolidated_annotations_KOHsmear.csv')
    # keep only "File name" and "Merged_numerical" columns
    df_annotated = df_annotated[['File name', 'Merged_numerical']]
    # remove "_cropped.jpeg" from "File name" end
    ...
    # V4 KEY CHANGE: Instead of filtering, remap all classes for test set
    print("Original test label distribution:")
    print(df_merged['label'].value_counts(dropna=False).sort_index())
    
    df_merged['label'] = df_merged['label'].apply(remap_labels_v4)
    
    print("After V4 test remapping (good vs poor quality):")
    print(df_merged['label'].value_counts(dropna=False).sort_index())
    
    # save new df as csv
    df_merged.to_csv(os.path.join('datasets', args.dataset_test, args.dataset_test+'_v4_edited.csv'), index=False)
    print('Test CSV length after V4 editing: ', len(df_merged))
    # define new bags_csv
    bags_csv_test = os.path.join('datasets', args.dataset_test, args.dataset_test+'_v4_edited.csv')
    #######################
    
    #######################
    # This code merges the same patients together by file name
    bags_path_test = pd.read_csv(bags_csv_test)
    print("Test CSV length before merging: ", len(bags_path_test))
     
    # Apply the function to each row in the DataFrame to create a new column for the combined path
    bags_path_test['0'] = bags_path_test['0'].apply(create_combined_path)
    print('Test CSV length after merging: ', len(bags_path_test))
    # Now consolidate the rows with the same combined path
    # always have 1 win over 0 because 1 is fungus and if there 1 patch of fungus, the whole slide is.
    bags_path_test = bags_path_test.groupby('0').agg({'label': 'max'}).reset_index()
    print('Test CSV length after consolidation: ', len(bags_path_test))
    # drop nan
    bags_path_test = bags_path_test.dropna()
    print('Test CSV length after dropping nans: ', len(bags_path_test))
    print('V4 Quality - Merged same test entries: ')
    print(bags_path_test.head())
    # print value counts without excluding nans
    print('V4 Quality - Test value counts: ')
    print(bags_path_test['label'].value_counts(dropna=False))
    # save the new csv for logging
    bags_path_test.to_csv('merged_patients_v4_test.csv', index=False)
    
    # Create the new directory name by appending '_combined'
    new_dataset_dir = 'datasets/' + args.dataset_test + '_v4_combined'
    # Clone the directory structure of the original dataset
    ...
    print(f"V4 Quality Test dataset cloned and combined at: {new_dataset_dir}")
    
    bags_csv_test = os.path.join(new_dataset_dir,args.dataset_test+'_v4_combined.csv')
    ...
    if args.eval_scheme == '5-fold-cv-standalone-test':
        ...
        for fold, (train_index, test_index) in enumerate(kf.split(bags_path)):
            print(f"Starting V4 Quality CV fold {fold}.")
        ...
        print("Starting V4 Quality testing phase.")
        fold_predictions = []
        ...
        print("V4 Quality Mean AUC:", mean_auc)
        print("V4 Quality Fold results on CV (accuracy + auc):", fold_results)
        
        print("V4 Quality Fold predictions shape:", np.array(fold_predictions).shape)
        print("V4 Quality Mode result:", mode_result)
        print("V4 Quality Combined predictions before squeeze:", mode_result.mode[0])
        print("V4 Quality Combined predictions:", combined_predictions)
        ...
        fold_predictions_df.to_csv(f'test/test_predictions_v4_quality_per_fold_{datetime.datetime.now().strftime("%Y%m%d_%H%M")}.csv', index=False)
        ...
        results_df.to_csv(f'test/test_predictions_v4_quality_mode_{datetime.datetime.now().strftime("%Y%m%d_%H%M")}.csv', index=False)

        if args.num_classes > 1:
            # Compute Hamming Loss
            hammingloss = hamming_loss(test_labels, combined_predictions)
            print("V4 Quality Hamming Loss:", hammingloss)
            # Compute Subset Accuracy
            subset_accuracy = accuracy_score(test_labels, combined_predictions)
            print("V4 Quality Subset Accuracy (Exact Match Ratio):", subset_accuracy)
        else:
            accuracy = accuracy_score(test_labels, combined_predictions)
            print("V4 Quality Accuracy:", accuracy)
            balanced_accuracy = balanced_accuracy_score(test_labels, combined_predictions)
            print("V4 Quality Balanced Accuracy:", balanced_accuracy)

        os.makedirs('test', exist_ok=True)
        with open("test/test_list_v4_quality.json", "w") as file:
            json.dump(reserved_testing_bags, file)

        for i, item in enumerate(fold_models):
            best_model = item[0]
            optimal_thresh = item[1]
            torch.save(best_model.state_dict(), f"test/mil_weights_v4_quality_fold_{i}.pth")
            with open(f"test/mil_threshold_v4_quality_fold_{i}.json", "w") as file:
